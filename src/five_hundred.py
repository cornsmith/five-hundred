...
class Bid(object):
    ...
    possible.insert(possible.index('7N'), 'CM')
...
class Round(object):
    ...
    def play_card(self, hand_index, trick):
...
class Trick(Hand):
    ...
    Args:
        ...
        misere: indicates a misere round

    Attributes:
        lead (int): player index of lead player
        misere (bool): indicates a misere round
    ...
    def __init__(self, lead, number, misere=False):
        self.lead = lead
        self.number = number
        self.misere = misere
        self.lead_suit = None
        ...
        if self.is_complete():
            ...
            for card in self.cards:
                try:
                    ...
                    lead_value = 0
                card_values.append(max(lead_value, card.value()))
            self.winner = (self.lead + card_values.index(max(card_values))) % 4

    def is_complete(self):
        """Returns whether all players have played their cards in trick"""
        return len(self.cards) + self.misere == 4
...
class Game(object):
    ...
    def start_round(self):
        ...
        self.deal()
        self.round.starting_hands = [player.cards[:] for player in players] + [self.kitty.cards[:]]
        
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')
        print('Round %s' % (self.round_number))
        print('~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~')

    def bid_round(self, policy=None):
        ...
        while br.status == "Bidding in progress":
            ...
            if policy == "random" and br.turn != HUMAN_PLAYER:
                try:
                    bid_text = random.choice(br.possible_bids)
                except IndexError:
                    bid_text = ''
    ...
    def discard_round(self, policy=None):
        ...
        if policy == "random" and dr.highest_bidder != HUMAN_PLAYER:
    ...
    def card_round(self, policy=None):
        """Starts a round of card playing."""
        # setup
        cr = self.round
        cr.status = "Card play in progress"
        misere = cr.highest_bid.misere is not None
        ...
        for trick_num in range(10):
            ...
            while not trick.is_complete():
                ...
                if policy == "random" and cr.turn != HUMAN_PLAYER:
                    hand_index = random.choice(self.players[cr.turn].possible_index)
                else:
                    print('\n--------------------------')
    ...
    def print_hands(self):
        ...
        for player in players:
            player.sort()
            if HUMAN_PLAYER == players.index(player) or HUMAN_PLAYER is None:
...
if __name__ == '__main__':
    ...
    parser.add_argument(
        '--ai',
        type=str,
        help="AI setting {None - all human; random (default) - random moves}",
        default="random"
    )
    parser.add_argument(
        '--play',
        type=int,
        help="Human player {1, 2, 3, 4}",
        default=None
    )
    parser.add_argument(
        '--savedir',
    ...
    while game.status == "In progress":
        ...
        # bidding
        game.bid_round(policy=POLICY)
        if game.round.status == "Bidding complete":
            trump_suit = game.round.trump_suit

            game.print_hands()
            game.discard_round(policy=POLICY)
            game.print_hands()

            # card play
            game.card_round(policy=POLICY)
